...
$(document).ready(function () {
  ...
  $('.instituicao').change(function () {
        ...
        $.getJSON(window.base_url + '/webresources/campus/get_campus_list/' + id_instituicao, {}, function (data) {
            $.each(data, function () {
                $('.campus').append($('<option />').val(this.idCampus).text(this.nome));
                $('.escolher_campus').append($('<option />').val(this.idCampus).text(this.nome));
            });
        });
  });//#instituicao.change()
  ...
  $('.campus').change(function () {
        ...
        $.getJSON(window.base_url + '/webresources/curso/get_curso_list/' + id_campus, {}, function (data) {
            $.each(data, function () {
                $('.curso').append($('<option />').val(this.idCurso).text(this.nome));
                $('.escolher_curso').append($('<option />').val(this.idCurso).text(this.nome));
            });
        });
  });//#campus.change()
  ...
  //****************************************************
  // Evento click do botão Escolher de um dos modais:
  ...
  $('div.modal-footer > button.btn-default[type!=reset]').click(function () {
        ...
        var nome = select.attr('id').replace(/(\w+)\:/g, '');
        nome = nome.substr(9);
  ...
  $('#cadastrarInstituicaoForm').submit(function (e) {
        var dados = {};
        dados.nome = this.nome.value;
        dados.sigla = this.sigla.value;
        dados.cidade = this.cidade.value;
        dados.estado = this.estado.value;
        dados.site = this.site.value;
        dados.tipo = this['cadastrarInstituicaoForm:tipo'].value;
        
        $.post(window.base_url + '/webresources/instituicao/create', dados, function (data) {
            ...
            $('.instituicao').append($('<option />').val(data.idInstituicao).text(data.sigla+" - "+data.nome));
            $('.instituicao option[value=' + data.idInstituicao + ']').prop('selected', true);
            $('.escolher_instituicao').append($('<option />').val(data.idInstituicao).text(data.sigla+" - "+data.nome));
            $('.escolher_instituicao option[value=' + data.idInstituicao + ']').prop('selected', true);
  ...
  $('#cadastrarCampusForm').submit(function (e) {
        ...
        $.post(window.base_url + '/webresources/campus/create', dados, function (data) {
            
            //Adiciona o campus recém cadastrado na SELECT do formulário de inscrição do papel
            //e na SELECT da modal_escolher_campus.
            $('.campus').append($('<option />').val(data.idCampus).text(data.nome));
            $('.campus option[value=' + data.idCampus + ']').prop('selected', true);
            $('.escolher_campus').append($('<option />').val(data.idCampus).text(data.nome));
            $('.escolher_campus option[value=' + data.idCampus + ']').prop('selected', true);
  ...
  $('#cadastrarCursoForm').submit(function (e) {
        var dados = {};
        dados.nome = this.nome.value;
        dados.nivel = this['cadastrarCursoForm:nivel'].value;
        dados.campus = $('.campus').val();
        $.post(window.base_url + '/webresources/curso/create', dados, function (data) {
            
            //Adiciona o curso recém cadastrado na SELECT do formulário de inscrição do papel
            //e na SELECT da modal_escolher_curso.
            $('.curso').append($('<option />').val(data.idCurso).text(data.nome));
            $('.curso option[value=' + data.idCurso + ']').prop('selected', true);
            $('.escolher_curso').append($('<option />').val(data.idCurso).text(data.nome));
            $('.escolher_curso option[value=' + data.idCurso + ']').prop('selected', true);
            //Limpara o formulário da modal e fecha a modal.
